...
impl RateLimiter {
    ...
    pub fn verify_packet<'a, 'b>(
        ...
        | Packet::HandshakeResponse(HandshakeResponse { sender_idx, .. }) = packet
        {
            ...
            // compare first 16 bytes of computed_mac1 against mac1 in constant time
            ...
            if self.is_under_load() {
                ...
                // Compare the first 16 bytes of computed_mac2 against mac2
                if computed_mac2[..16].ct_eq(mac2).unwrap_u8() != 1 {
                    // MAC mismatch → send cookie reply
